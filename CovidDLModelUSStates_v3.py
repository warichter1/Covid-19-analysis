...
class CovidCountryRegion:
    ...
    def setRegionKeys(self):
        """Build data structure for each region."""
        for region in self.regions:
            self.dataStore[region] = {'confirmed': [], 'deaths': [],
                                      'confirmedNew': [], 'deathsNew': [],
                                      'totalTests': [], 'casesPerCapita': [],
                                      'caseRate': [], 'deathRate': [],
                                      'maxCaseRate': 0, 'recovered': [],
                                      'maxDeathRate': 0, 'control': 'na',
                                      'increasingDeaths': False,
                                      'population': 0, 'totalTests': [],
                                      'increasingCases': False, 'positive': [],
                                      'negative': [], 'pending': [],
                                      'hospitalizedCumulative': [],
    ...
    def processRegionResults(self, region, store):
        ...
        for day in self.daysIndex:
            ...
            # print(results)
            # print(dict(dataStore))
            for key in self.trackingList:
                dataStore[region][key].append(results[key])
            confirmed = self.confirmed.loc[region][day].sum()
            dataStore[region]['confirmed'].append(confirmed)
            # print('Confirmed', dataStore[region]['confirmed'])
            dataStore[region]['deaths'].append(self.deaths.loc[region][day].sum())
            if previousDay == "":
                dataStore[region]['caseRate'].append(0)
                dataStore[region]['deathRate'].append(0)
                dataStore[region]['casesPerCapita'].append(0)
                dataStore[region]['confirmedNew'].append(confirmed)
                dataStore[region]['deathsNew'].append(self.deaths.loc[region][day].sum())
            else:
                if dataStore[region]['confirmed'][-2:][0]  > 0:
                    caseRate = abs(dataStore[region]['confirmed'][-2:][0] / confirmed - 1)
                    dataStore[region]['caseRate'].append(caseRate)
                    if region in self.censusPop.keys():
                        dataStore[region]['casesPerCapita'].append(confirmed / self.censusPop[region])
                    else:
                        dataStore[region]['casesPerCapita'].append(0)
                    if caseRate >= dataStore[region]['maxCaseRate']:
                        self.dataStore[region]['maxCaseRate'] = caseRate
                    if caseRate > dataStore[region]['caseRate'][-2:][0]:
                        dataStore[region]['increasingCases'] = True
                    else:
                        dataStore[region]['increasingCases'] = False
                else:
                    dataStore[region]['caseRate'].append(0)
                    dataStore[region]['casesPerCapita'].append(0)
                ...
                if dataStore[region]['deaths'][-2:][0]  > 0:
                    ...
                    if deathRate > dataStore[region]['deathRate'][-2:][0]:
                        dataStore[region]['increasingDeaths'] = True
                    else:
                        dataStore[region]['increasingDeaths'] = False
                else:
                    dataStore[region]['deathRate'].append(0)
    ...
    def getPop(self, region):
        if region in self.censusPop.keys():
            print("Checking Census for: ", region)
